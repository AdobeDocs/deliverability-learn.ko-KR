...
Adobe에서는 Adobe 게재 팀이 고유한 프로그램을 이해할 수 있도록 고객이 구성을 공유해야 합니다. 질문들은 Adobe 전달성 팀이 귀하의 전송 평판과 이메일 볼륨을 파악할 수 있도록 고안되었습니다. 비즈니스 모델, 이메일 마케팅 목표 및 신뢰도 지표에 대한 구체적인 이해가 없으면 전략을 사용자 정의할 수 없으며 게재 가능성 문제가 발생할 수 있습니다.

처음에는 전용 IP(인터넷 프로토콜) 주소가 할당됩니다. 이메일 전송 맥락에서 IP 주소는 고객에게 이메일 메시지를 전달하는 데 사용되는 경로입니다. IP 주소 및 도메인은 네트워크에서 수신 ISP로 보낸 사람을 식별하는 데 사용됩니다. Adobe은 전송 볼륨, 이메일 프로그램, 데이터 세분화 사례 및 계약에 따라 이메일을 전송하기 위해 적절한 수의 전용 IP 주소를 할당합니다.
...
대답은 &#39;예&#39;일 가능성이 높습니다. 이 사람과 그 동기를 분석하고 싶을 것입니다. 해를 끼치겠다는 뜻인가요? 위협인가? ISP는 악성 트래픽이나 원치 않는 트래픽으로부터 네트워크를 보호하기 위해 동일한 작업을 수행합니다. 긍정적인 신뢰도 지표는 성공적인 IP 준비 과정에서 큰 도움이 됩니다. 따라서 소량의 이메일 전송부터 시작하여 참여도가 높은 고객에게 먼저 전송하는 것이 중요하다고 강조합니다. 자세한 내용은 [새 트래픽을 보낼 때의 타깃팅 기준](/help/transition-process/targeting-criteria.md)을 참조하십시오.

새로운 IP 또는 IP에서 대량의 이메일을 게이트 밖으로 보내는 것은 부적절한 방법이며 일부 전달성에 문제가 발생할 수 있습니다. 작은 볼륨을 보내기 시작하고 권장 사항대로 점진적으로 늘리는 경우에도 이메일 모범 사례를 따라야 합니다.

![](../../help/assets/ip-warming-volume-trend.png)

## 메일 권한(명시적 옵트인)

구독자 이메일 목록을 관리하고 확장하는 데 가장 중요한 구성 요소입니다. 스팸 방지법이 증가하고 국제적으로 더욱 포괄적이 됨에 따라 마케팅 담당자의 주된 초점이 되어 목록에 있는 각 구독자로부터 명시적인(또는 명시적인) 동의를 받았는지 확인해야 합니다. 즉, 각 구독자는 귀하의 브랜드로부터 이메일을 받는 데 적극적으로 동의했습니다. 이는 이메일 프로그램에 명시적으로 등록하지 않은 작업을 수행한 후 이메일 목록에 개인이 추가되는 경우의 암시적 동의와 다릅니다.
...
ISP에는 해당 정책 및 모범 사례에 맞는 특정 기술 구성이 필요합니다. Adobe은 귀하를 신뢰할 수 있는 발신자로 식별하기 위해 IP 및 위임된 하위 도메인을 구성합니다. 이를 [전자 메일 인증](/help/transition-process/infrastructure.md#authentication)이라고 합니다. 인증은 수신자가 해당 IP 또는 도메인에서 보낼 권한이 있는지 확인하는 데 도움이 됩니다.

인증을 통해 ISP는 도메인 또는 IP에서 보내는 회사에 권한이 있는지 확인할 수 있습니다. 그것은 본질적으로 당신의 신분을 증명하기 위해 그리고 당신이 다른 사람으로 행세하지 않고, 또 다른 누군가가 당신을 행세하지 않고 있다는 것을 확실히 하기 위해 행해집니다.

Adobe에서는 기본적으로 SPF 및 DKIM을 구성하고 요청별로 DMARC을 구성합니다. ISP는 주요 인증 형태로 SPF 및 DKIM을 참조합니다. 많은 ISP가 또한 필터링 결정에 DMARC(도메인 기반 메시지 인증, 보고 및 적합성)을 통합하고 있습니다. 인증되지 않은 이메일은 반드시 차단되지는 않지만 추가 필터링을 거칩니다.
...
가입자들은 여러 가지 이유로 불평을 한다. 구독자가 동일한 주제에 대해 너무 많은 메시지가 전송된다고 생각하거나, 메시지가 표시될 것으로 예상하지 못했거나, 이메일을 수신하기 위해 등록한 기억이 없기 때문에 더 이상 이메일을 받고 싶지 않을 때가 있습니다.
...
하드 바운스가 상당히 많이 발생하는 경우 그 이유를 이해하는 것이 중요합니다. 주소 수집 방법을 검토하고 권한이 부여되었는지 확인합니다. 때때로 사람들은 이메일 계정을 닫고 마케팅 목록에 해당 주소가 있는 사람에게 알리지 않습니다.
